...
export const StatBar: React.FC<Props> = ({ icon, label, value, type, max = 100, change }) => {
  ...
  return (
    <div className="bg-black/20 rounded-lg p-2 md:p-3 lg:p-4 flex items-center justify-between shadow-sm backdrop-blur-sm relative overflow-hidden group">
      <div className="flex items-center space-x-2 w-1/3 z-10">
        <span className="text-xl lg:text-3xl filter drop-shadow-md">{icon}</span>
        <span className="text-gray-200 font-medium text-sm lg:text-lg">{label}</span>
      </div>
      
      {isSpecial ? (
        <div className="flex-1 flex justify-end items-center space-x-2 z-10">
           {change !== undefined && change !== 0 && (
            <span className={`text-sm lg:text-base font-bold animate-bounce ${change > 0 ? 'text-green-400' : 'text-red-400'}`}>
              {change > 0 ? '+' : ''}{change}
            </span>
          )}
          <div className={`font-bold text-lg lg:text-2xl ${getColor(type)} text-right`}>
            {type === 'money' ? `¥${value.toLocaleString()}` : `${value}kg`}
          </div>
        </div>
      ) : (
        <div className="flex-1 flex items-center space-x-3 z-10">
          <div className="flex-1 h-3 lg:h-4 bg-gray-800/50 rounded-full overflow-hidden border border-white/5 relative">
             <div 
               className={`h-full rounded-full transition-all duration-500 ease-out shadow-[0_0_10px_rgba(0,0,0,0.3)] ${getBarColor(type)}`} 
               style={{ width: `${percentage}%` }}
             ></div>
          </div>
          <div className="w-16 lg:w-20 flex justify-end items-center space-x-1">
             {change !== undefined && change !== 0 && (
                <span className={`text-xs lg:text-sm font-bold animate-pulse ${change > 0 ? 'text-green-400' : 'text-red-400'} absolute -top-1 right-2`}>
                  {change > 0 ? '+' : ''}{change}
                </span>
             )}
             <span className={`font-bold text-right text-base lg:text-xl ${getColor(type)}`}>{value}</span>
          </div>
        </div>
      )}
    </div>

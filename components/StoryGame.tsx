...
export const StoryGame: React.FC<Props> = ({ character, currentStats, event, onChoiceMade }) => {
  ...
  const getTimeIcon = (time: string) => {
    const iconClass = "w-4 h-4 lg:w-6 lg:h-6 mr-1 lg:mr-2";
    switch (time) {
  ...
  const handleChoice = (choice: Choice) => {
    ...
    // Duration needs to match or slightly exceed the Toast's display logic if we want to block interaction
    // The Toast component handles its own visibility timer, but here we wait to unmount/transition.
    // Reduced delay to 3500ms to improve pacing.
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-festive-dark p-2 md:p-6 lg:p-8 flex items-center justify-center">
      <Toast 
      ...
      {/* Main Container */}
      <div className="flex flex-col md:flex-row gap-4 md:gap-6 lg:gap-10 w-full max-w-7xl lg:max-w-[95vw] xl:max-w-[1800px] h-[95vh] md:h-[90vh]">
        
        {/* Left Column: Stats & Profile (Fixed width on desktop, scaled on LG) */}
        ...
        <div className="flex-1 flex flex-col h-full bg-festive-bg rounded-2xl overflow-hidden shadow-2xl border-4 border-[#5E1212] relative">
          ...
          {/* Top Section: Image & Description (Overlay) */}
          <div className="relative h-1/3 lg:h-[35%] w-full flex-shrink-0 group overflow-hidden">
             {/* Background Image */}
             <img 
               src={event.image} 
               alt={event.title} 
             ...
             {/* Content Overlay */}
             <div className="absolute bottom-0 left-0 right-0 p-4 lg:p-8 flex flex-col justify-end h-full pointer-events-none">
                <div className="flex flex-col justify-end h-full pointer-events-auto">
                  <div className="flex items-center space-x-3 mb-2 lg:mb-4">
                    <span className="bg-red-600 text-white text-xs lg:text-base font-bold px-2 py-0.5 lg:px-3 lg:py-1 rounded shadow-sm border border-red-400">
                      事件
                    </span>
                    <h3 className="text-xl md:text-2xl lg:text-4xl font-bold text-white drop-shadow-md tracking-wide line-clamp-1">
                      {event.title}
                    </h3>
                  </div>
                  
                  {/* Description Box - Scrollable if text is too long */}
                  <div className="bg-white/10 backdrop-blur-md border border-white/20 p-3 lg:p-6 rounded-xl shadow-lg max-h-[85%] overflow-y-auto custom-scrollbar">
                    <p className="text-gray-100 text-sm md:text-base lg:text-xl xl:text-2xl leading-relaxed lg:leading-loose font-medium drop-shadow-sm text-justify">
                      {event.description}
                    </p>
                  </div>
                </div>
          ...
          {/* Bottom Section: Choices (Expanded Area) */}
          <div className="flex-1 h-2/3 lg:h-[65%] bg-[#FFF5E6] relative flex flex-col">
            {/* Divider Line */}
            <div className="h-1 bg-gradient-to-r from-yellow-600 via-yellow-400 to-yellow-600 shadow-md z-10"></div>
            
            <div className="flex items-center justify-center py-2 lg:py-3 bg-[#FFF0D4] border-b border-yellow-200/50 flex-shrink-0">
              <span className="text-yellow-800/60 text-xs lg:text-sm font-bold uppercase tracking-[0.2em]">— 请做出你的选择 —</span>
            </div>

            <div className="flex-1 p-3 md:p-4 lg:p-6 flex flex-col justify-center gap-3 md:gap-4 lg:gap-4 overflow-y-auto custom-scrollbar">
              {event.choices.map((choice, index) => (
                <button
                  ...
                  className={`
                    w-full relative group text-left p-3 md:p-4 lg:p-5 rounded-xl md:rounded-2xl border-2 transition-all duration-300 transform flex-1 min-h-[80px] lg:min-h-[100px] flex flex-col justify-center
                ...
                >
                  <div className="flex items-center h-full">
                    ...
                    <div className={`
                      flex-shrink-0 w-8 h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 rounded-full flex items-center justify-center font-bold text-white mr-3 lg:mr-5 shadow-md text-base md:text-lg lg:text-xl transition-colors duration-300
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <div className="font-bold text-gray-800 text-base md:text-lg lg:text-2xl mb-0.5 lg:mb-1 group-hover:text-red-800 transition-colors">
                        {choice.title}
                      </div>
                      <div className="text-gray-600 text-xs md:text-sm lg:text-lg leading-snug lg:leading-normal line-clamp-2 lg:line-clamp-3 font-medium">
                        {choice.description}
                      </div>
                    </div>
                  </div>
